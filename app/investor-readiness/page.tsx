...
const FUNDING_AMOUNTS = ["₦52.5m · 35%", "₦37.5m · 25%", "₦37.5m · 25%", "₦15m · 10%", "₦7.5m · 5%"];
    ...
    <main className="min-h-screen bg-[#f7f7f2] text-[#0d1f19] dark:bg-[#07110e] dark:text-white">
      ...
      <section className="border-b border-black/8 py-20 dark:border-white/10 sm:py-28">
        <div className="mx-auto grid max-w-7xl gap-14 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
          ...
          <article><Banknote className="h-7 w-7 text-emerald-700 dark:text-[#d9ff57]" /><h2 className="mt-7 font-display text-4xl font-black leading-tight">{copy.fundingTitle}</h2><p className="mt-5 text-sm leading-7 text-slate-600 dark:text-white/58">{copy.fundingBody}</p><div className="mt-8 border-y border-black/12 dark:border-white/12">{copy.fundingLabels.map((label,index) => <div key={label} className={`grid grid-cols-[1fr_auto] gap-4 py-3.5 text-sm ${index ? "border-t border-black/8 dark:border-white/8" : ""}`}><span className="font-bold">{label}</span><span className="font-mono text-right font-black text-emerald-700 dark:text-[#d9ff57]">{FUNDING_AMOUNTS[index]}</span></div>)}<div className="grid grid-cols-[1fr_auto] border-t-2 border-[#071713] py-5 dark:border-white"><span className="font-black">TOTAL</span><span className="font-mono text-xl font-black">₦150m</span></div><p className="border-t border-black/10 py-3 text-xs text-slate-500 dark:border-white/10 dark:text-white/45">Approximately US$95k at the planning exchange rate; the naira amount is the governing ask.</p></div></article>
        </div>
      </section>
