...
function asRecord(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("observation must be a JSON object");
...
function metadata(value: unknown) {
  ...
  for (const [key, item] of Object.entries(obj)) {
    if (item == null || ["string", "number", "boolean"].includes(typeof item)) clean[key] = item as string | number | boolean | null;
    else throw new Error(`metadata.${key} must be a primitive JSON value`);
  }
  return clean;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("Multi-source ingestion auth/storage error", error);
    return NextResponse.json({ error: "Multi-source ingestion storage is not ready." }, { status: 503 });
  }
  ...
  try { root = asRecord(body); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Invalid body" }, { status: 400 }); }

  const rawObservations = root.observations;
  if (!Array.isArray(rawObservations) || rawObservations.length === 0) return NextResponse.json({ error: "observations must be a non-empty array." }, { status: 400 });
  if (rawObservations.length > MAX_RECORDS) return NextResponse.json({ error: `Maximum ${MAX_RECORDS} observations per request.` }, { status: 413 });
  ...
  for (let index = 0; index < rawObservations.length; index++) {
    try {
      ...
      if (!QUALITY.has(rawQuality as QualityStatus)) throw new Error(`unsupported qualityStatus: ${raw.qualityStatus}`);
      const flags = raw.flags == null ? undefined : Array.isArray(raw.flags) && raw.flags.every((x) => typeof x === "string") ? (raw.flags as string[]) : (() => { throw new Error("flags must be an array of strings"); })();
      accepted.push(normalizeObservation({
      ...
      rejected.push({ index, error: error instanceof Error ? error.message : "Invalid observation" });
    }
  }

  if (!accepted.length) return NextResponse.json({ error: "No valid observations supplied.", rejected }, { status: 400 });

  try {
    const persisted = await persistCanonicalObservations(auth.source.slug, accepted);
    return NextResponse.json({
      source: { slug: auth.source.slug, provider: auth.source.provider, kind: auth.source.sourceKind },
    ...
    console.error("Multi-source ingestion persistence error", error);
    return NextResponse.json({ error: "Multi-source ingestion persistence is unavailable." }, { status: 503 });
  }
}
