...
import { ArrowDown, ArrowRight, ArrowUpRight, BadgeCheck, Clock3, MapPin, Radio, ShieldCheck, Waves } from "lucide-react";
import FpShell, { useFp } from "@/components/floodpass/FpShell";
...
function HomeBody() {
  const { lang } = useFp(); const [coverage, setCoverage] = useState<Coverage | null>(null); const [recent, setRecent] = useState<Recent | null>(null);
  useEffect(() => {
  ...
  return <div className="ncg-home">
    <section className="ncg-hero" aria-labelledby="ncg-hero-title">
      <div className="ncg-hero-copy"><div className="ncg-eyebrow"><span className="ncg-signal" /> NAIJACLIMAGUARD / FLOOD INTELLIGENCE FOR PEOPLE</div>
        ...
        <p>{lang === "pcm" ? "Know the flood warning for your area. Report wetin you see. Keep a record wey other people fit check." : "See the warning for your area. Report the water you see. Keep a record others can check. One clear path from risk to action."}</p>
