...
export default function PitchFilm() {
  ...
  return (
    <>
      <div className="group relative aspect-video overflow-hidden rounded-[2rem] border border-black/10 bg-[#102820] shadow-[0_24px_80px_rgba(3,25,18,.22)] dark:border-white/15">
        {playing ? <FilmFrame title="NaijaClimaGuard two-minute concept film" /> : <>
          <img src={`https://i.ytimg.com/vi/${FILM_ID}/maxresdefault.jpg`} alt="NaijaClimaGuard concept-film cover" className="absolute inset-0 h-full w-full object-cover" />
        ...
        {playing && <button onClick={() => setExpanded(true)} className="absolute right-3 top-3 z-10 flex h-10 items-center gap-2 rounded-full bg-black/75 px-3 text-xs font-black text-white backdrop-blur" aria-label="Expand film"><Expand className="h-4 w-4" /> Expand</button>}
      </div>
      ...
      {expanded && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020906]/94 p-3 backdrop-blur-xl sm:p-8" role="dialog" aria-modal="true" aria-label="Expanded NaijaClimaGuard concept film">
        <button ref={closeButton} onClick={() => window.history.back()} className="absolute right-4 top-4 z-20 flex h-11 items-center gap-2 rounded-full bg-white px-4 text-xs font-black text-[#071713]" aria-label="Close expanded film"><X className="h-4 w-4" /> Back to pitch</button>
        <div className="relative aspect-video w-full max-w-6xl overflow-hidden rounded-[1.5rem] bg-black shadow-2xl"><FilmFrame title="Expanded NaijaClimaGuard two-minute concept film" /></div>
      </div>}
    </>
  );
}
