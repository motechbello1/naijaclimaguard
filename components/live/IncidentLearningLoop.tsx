...
export default function IncidentLearningLoop() {
  ...
  const loadForecastReplay = async () => {
    ...
    try {
      ...
      // Historical run replay is enrichment; the live incident feed should remain usable if it is temporarily unavailable.
    ...
    <section className="space-y-4">
      ...
      {forecastReplay ? (
        <div className="rounded-2xl border border-radar/20 bg-radar/[0.04] p-4 sm:p-5">
          ...
          <p className="mt-3 text-[11px] leading-5 text-slate-500">This replays only forecast information that would have been available before the report, with a conservative model-publication delay. It does not use later observations to make an old forecast look better.</p>
        </div>
      ) : forecastLoading ? <div className="rounded-xl border border-slate-200 p-4 text-sm text-slate-500 dark:border-midnight-border">Replaying pre-event ECMWF forecast runs…</div> : null}
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {audits.map((audit) => {
          ...
          return <article key={`${audit.state}-${audit.firstReportAt}`} className={`glass-card rounded-2xl p-4 ${miss ? "border-amber/20" : ""}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">{audit.state} · {audit.sentinel} · {audit.pointsChecked || 1} point{audit.pointsChecked === 1 ? "" : "s"}</p><h3 className="mt-1 text-sm font-bold leading-5 line-clamp-2">{audit.firstReportTitle}</h3><p className="mt-1 text-xs text-slate-500">{audit.firstReportSource} · {new Date(audit.firstReportAt).toLocaleString("en-NG")}</p></div>{miss ? <TriangleAlert className="h-5 w-5 shrink-0 text-amber" /> : <CheckCircle2 className="h-5 w-5 shrink-0 text-radar" />}</div>{audit.observedRainfallAudit ? <div className="mt-4 grid grid-cols-4 gap-2"><Stat label="Audit score" value={audit.observedRainfallAudit.score} /><Stat label="1h rain" value={`${audit.observedRainfallAudit.features.rain_1h_mm} mm`} /><Stat label="3h rain" value={`${audit.observedRainfallAudit.features.rain_3h_mm} mm`} /><Stat label="24h rain" value={`${audit.observedRainfallAudit.features.rain_24h_mm} mm`} /></div> : null}<p className="mt-3 text-xs leading-5 text-slate-500">{audit.explanation}</p></article>;
        })}
      </div>
      <p className="text-xs leading-5 text-slate-500">A miss can expose a highly local cloudburst, drainage or terrain vulnerability, a flooded location outside the current grid, or thresholds that need retraining. We keep those possibilities separate instead of automatically blaming or crediting the ML model.</p>
    </section>
  );
}
