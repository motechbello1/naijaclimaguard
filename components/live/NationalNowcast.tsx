    ...
    <section className="space-y-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500 dark:text-slate-400">One rainfall sentinel at every state capital and the FCT checks the latest 1, 3, 6, 24, 72 and 168 hour rainfall, plus the next six hours. This is the layer intended to notice dangerous rain before a newspaper reports the damage.</p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-midnight-border dark:bg-midnight-light">
        <div className="grid gap-3 sm:grid-cols-3">
          <div><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Sentinels online</p><p className="mt-1 font-mono text-2xl font-bold">{sentinels.filter((item) => item.available).length || "—"}</p></div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {top.map((item) => {
          ...
          return <div key={item.state} className="glass-card rounded-2xl p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-slate-400">{item.state}</p><p className="font-display text-base font-bold">{item.city}</p></div><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${levelStyle[risk.level]}`}>{risk.level} · {risk.score}</span></div><div className="mt-4 grid grid-cols-4 gap-2 text-center"><RainStat label="1h" value={risk.features.rain_1h_mm} /><RainStat label="3h" value={risk.features.rain_3h_mm} /><RainStat label="24h" value={risk.features.rain_24h_mm} /><RainStat label="72h" value={risk.features.rain_72h_mm} /></div><p className="mt-3 flex items-start gap-2 text-xs leading-5 text-slate-500"><CloudRain className="mt-0.5 h-3.5 w-3.5 shrink-0 text-radar" /> {risk.drivers[0]}</p></div>;
        })}
      </div>

      <p className="text-xs leading-5 text-slate-500">Coverage note: each card is a capital-city screening sentinel, not a declaration for the whole state. The next expansion is multiple urban hotspots per state, drainage/terrain exposure and verified road-segment incidents.</p>
    </section>
  );
}
