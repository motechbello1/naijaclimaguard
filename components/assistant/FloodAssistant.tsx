...
export default function FloodAssistant() {
  ...
  return (
    <>
      ...
      {open && (
        <>
          <button aria-label={t("close")} onClick={() => setOpen(false)} className="fixed inset-0 z-[88] hidden bg-slate-950/35 backdrop-blur-[1px] sm:block" />
          <section className="fixed inset-0 z-[90] flex h-[100dvh] flex-col overflow-hidden bg-white shadow-2xl dark:bg-midnight sm:inset-auto sm:bottom-4 sm:right-4 sm:h-[min(680px,calc(100dvh-2rem))] sm:w-[min(420px,calc(100vw-2rem))] sm:rounded-2xl sm:border sm:border-slate-200 dark:sm:border-midnight-border">
            <header className="flex shrink-0 items-center justify-between border-b border-slate-200 px-3 py-2.5 pt-[max(0.65rem,env(safe-area-inset-top))] dark:border-midnight-border sm:px-4 sm:pt-3">
              ...
              <div className="flex shrink-0 items-center gap-0.5">
                <button onClick={() => setMessages([])} className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800" title={t("newChat")}><RotateCcw className="h-4 w-4" /></button>
                <button onClick={() => setOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800" title={t("close")}><X className="h-4 w-4" /></button>
              </div>
            </header>

            <div className="shrink-0 border-b border-slate-200 bg-radar/5 px-3 py-2 text-[11px] leading-4 text-slate-600 dark:border-midnight-border dark:text-slate-300 sm:px-4">
              {locale === "pcm" ? "Official warning and flood wey you see for ground get priority over chatbot advice." : locale === "ha" ? "Gargadin hukuma da ambaliyar da kake gani sun fi shawarar chatbot muhimmanci." : locale === "yo" ? "Ìkìlọ̀ ìjọba àti ìkún omi tí o rí ló ga ju ìmọ̀ràn chatbot lọ." : locale === "ig" ? "Official warning na flood ị na-ahụ dị mkpa karịa chatbot advice." : "Official warnings and visible local flooding take priority over chatbot advice."}
            </div>

            <div className="flex-1 space-y-3 overflow-y-auto overscroll-contain px-3 py-3 sm:px-4">
              {messages.length === 0 && (
                <div className="space-y-2.5">
                  <button onClick={checkNearMe} disabled={busy} className="flex min-h-11 w-full items-center gap-2 rounded-xl border border-radar/20 bg-radar/5 p-3 text-left text-sm font-semibold hover:border-radar/40">
                  ...
                  {starters.map((starter) => (
                    <button key={starter} onClick={() => ask(starter)} className="min-h-11 w-full rounded-xl border border-slate-200 p-3 text-left text-sm hover:border-radar/40 dark:border-midnight-border">{starter}</button>
                  ))}
                </div>
              )}
              ...
              {messages.map((message) => (
                <div key={message.id} className={`flex flex-col ${message.role === "user" ? "items-end" : "items-start"}`}>
                  <div className={`max-w-[86%] break-words rounded-2xl px-3.5 py-2.5 text-[15px] leading-6 whitespace-pre-line sm:max-w-[88%] sm:text-sm ${message.role === "user" ? "bg-radar text-white" : "bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-100"}`}>
                    {message.text}
                  </div>
                  {message.role === "assistant" && message.sourceClass && (
                    <div className="mt-1.5 flex items-center gap-1.5 px-1 text-[10px] font-medium uppercase tracking-wide text-slate-400"><ShieldAlert className="h-3 w-3" /> {message.sourceClass}</div>
                  )}
                  {message.role === "assistant" && message.id === lastAssistantId && message.suggestions?.length ? (
                    <div className="mt-2 flex max-w-full flex-wrap gap-1.5">
                      {message.suggestions.map((suggestion) => (
                        <button key={suggestion} onClick={() => ask(suggestion)} disabled={busy} className="rounded-full border border-slate-200 px-2.5 py-1.5 text-left text-[11px] leading-4 text-slate-600 hover:border-radar/40 hover:text-radar dark:border-midnight-border dark:text-slate-300">{suggestion}</button>
                      ))}
                    </div>
                  ) : null}
                </div>
              ))}
              {busy && <div className="px-1 text-xs text-slate-400">{t("loading")}</div>}
              <div ref={bottomRef} />
            </div>

            <form onSubmit={submit} className="shrink-0 border-t border-slate-200 bg-white p-2.5 pb-[max(0.65rem,env(safe-area-inset-bottom))] dark:border-midnight-border dark:bg-midnight sm:p-3">
