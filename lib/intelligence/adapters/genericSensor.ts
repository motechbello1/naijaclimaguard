...
export function adaptGenericSensorRecord(record: Record<string, unknown>, config: GenericSensorAdapterConfig): CanonicalObservation {
  ...
  const qualityStatus = ["good", "suspect", "stale", "missing"].includes(qualityText)
    ...
    : "unknown";
  const externalRecordId = config.fields.externalRecordId ? readPath(record, config.fields.externalRecordId) : undefined;
  return normalizeObservation({
    ...
    location,
    qualityStatus,
    metadata: { adapter: "generic-sensor-v1" },
  });
}
