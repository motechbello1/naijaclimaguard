...
export function observationDedupeKey(observation: CanonicalObservation): string {
  const identity = observation.provenance.externalRecordId
    ? [
        ...
        observation.provenance.externalRecordId,
      ]
    : [
        observation.provenance.provider,
        observation.provenance.sourceId,
...
/**
 * Registers/updates an integration source definition. This does not create an
 * external credential and does not expose an ingestion endpoint.
 */
export async function registerIntelligenceSource(input: RegisterSourceInput) {
  if (!input.slug.trim()) throw new Error("source slug is required");
  if (!Number.isFinite(input.defaultFreshnessMinutes) || input.defaultFreshnessMinutes <= 0) {
    throw new Error("defaultFreshnessMinutes must be positive");
  }
...
/**
 * Persists normalized observations immutably. Duplicate provider records are
 * ignored by deterministic dedupe key rather than overwritten.
 *
 * IMPORTANT: call this only after the Platform-v3 database migration is
 * explicitly applied. The current production database has not been migrated.
 */
export async function persistCanonicalObservations(
  sourceSlug: string,
  ...
  const result = await prisma.intelligenceObservation.createMany({
    data,
    skipDuplicates: true,
  });
