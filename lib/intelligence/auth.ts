...
export function issueIntelligenceKeyMaterial() {
  ...
  return { plaintext, keyHash: hashIntelligenceKey(plaintext), keyPrefix: plaintext.slice(0, 16) };
}

export async function createIntelligenceCredential(input: { sourceSlug: string; name: string; scopes?: string[] }) {
...
export type IntelligenceAuthResult =
  | { ok: true; credentialId: string; source: { id: string; slug: string; provider: string; name: string; sourceKind: SourceKind; active: boolean }; scopes: string[] }
  | { ok: false; status: 401 | 403 | 503; error: string };

export async function authenticateIntelligenceRequest(request: Request, requiredScope = "ingest"): Promise<IntelligenceAuthResult> {
  if (process.env.PLATFORM_V3_INGESTION_ENABLED !== "true") return { ok: false, status: 503, error: "Multi-source ingestion is not enabled." };
  ...
  const credential = await prisma.intelligenceCredential.findUnique({
    where: { keyHash: hashIntelligenceKey(token) },
    include: { source: true },
  });
  if (!credential || !credential.active) return { ok: false, status: 401, error: "Invalid or inactive service credential." };
  if (!credential.source.active) return { ok: false, status: 403, error: "Registered intelligence source is disabled." };
  const scopes = Array.isArray(credential.scopes) ? credential.scopes.filter((scope): scope is string => typeof scope === "string") : [];
  if (!scopes.includes(requiredScope)) return { ok: false, status: 403, error: `Credential lacks required scope: ${requiredScope}` };
  await prisma.intelligenceCredential.update({ where: { id: credential.id }, data: { lastUsedAt: new Date() } });
  return {
    ok: true,
