...
export function computeImpactGraph(assets: ImpactAsset[], decisionScore: number, _officialWarning: boolean): ImpactNode[] {
  ...
  return assets.filter((asset) => asset.enabled).map((asset): ImpactNode => {
...
const ROLE_ACTIONS: Record<DecisionRole, DecisionAction[]> = {
  HOUSEHOLD: [
    { code: "household_people_first", timeframe: "now", label: "Confirm who needs help first", why: "Children, older people and people with mobility or medical needs should be prioritised before property.", priority: 100 },
...
export function compileDecisionActions(role: DecisionRole, impact: ImpactNode[], trust: TrustAssessment, officialWarning: boolean): DecisionAction[] {
  ...
  if (highestExposure >= 60) {
    actions.push({ code: "impact_top_asset", timeframe: "now", label: `Protect highest-priority exposure: ${impact[0]?.name ?? "critical asset"}`, why: `Its scenario exposure score is ${highestExposure}/100 based on the current hazard state and your vulnerability settings. This is not a probability.`, priority: 105 });
  }
  if (trust.band === "degraded" || trust.band === "insufficient") {
    actions.push({ code: "source_degraded", timeframe: "now", label: "Treat missing or stale feeds as uncertainty, not safety", why: trust.explanation, priority: 112 });
  }
  if (officialWarning) {
    actions.push({ code: "official_override", timeframe: "now", label: "Official instruction takes priority", why: "The source-trust policy never allows a model or user profile to suppress a fresh official warning.", priority: 140 });
  }
